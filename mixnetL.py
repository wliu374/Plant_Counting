...
class MixNet(nn.Module):
    ...
    def __init__(self,input_size=320,
        ...
        layers=[layers0,layers1,layers2]
        # print(config[0])
        # print(config[1])
        for j in range(3):
            for in_channels, out_channels, kernel_size, expand_ksize, project_ksize, stride, expand_ratio, non_linear, se_ratio in config[j]:

                # print([in_channels, out_channels, kernel_size, expand_ksize, project_ksize, stride, expand_ratio, non_linear, se_ratio])
                layers[j].append(MixNetBlock(
                    in_channels,
                    out_channels,
            ...
            # print(layers[j])
            # print(len(layers[j]))
        self.layers0 = nn.Sequential(*layers[0])
        self.layers1 = nn.Sequential(*layers[1])
        self.layers2 = nn.Sequential(*layers[2])
    ...
    def forward(self, x):
        ...
        one_16 = self.layers1(x)
        one_32 = self.layers2(one_16)


        return {"one_8":x,"one_16":one_16,"one_32":one_32}
    #
...
if __name__ == '__main__':
    net = MixNet().cuda()
    x_image = torch.randn(8, 3, 256, 256).cuda()
    dic = net(x_image)
    one_8=dic['one_8']
    one_16=dic['one_16']
    one_32=dic['one_32']
